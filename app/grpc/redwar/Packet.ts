...
export default class PacketGrpcService extends BaseGrpc {
  ...
  private async toController(controller) {
    ...
    try {
      const token = this.call.metadata.get('token')[0] as string;
      ...
      if (err.status === 422) {
        ctx.fail(
          ...
          this.app.config.env !== 'prod'
            ? err.errors[0].field + ': ' + err.errors[0].code + ', ' + err.errors[0].message
            : err.errors[0].field,
        );
      } else {
        ctx.fail(ctx.code.server_error, this.app.config.env !== 'prod' ? err.name + ':' + err.message : null);
      }
    }
