...
export default class UserController extends Controller {
  private userinfo: string[] = [ 'id', 'uid', 'account', 'name', 'headimgurl', 'award', 'share' ];
  ...
  public async index() {
    ...
    // 获取到的是user实例
    const user = await this.refresh(ctx.session.user.id);
  ...
  public async login() {
    ...
    if (password) {
      ...
      const auth = await ctx.auth(user.account, code);
      if (!auth) {
        return;
      }
    } else {
  ...
  public async register() {
    ...
    // 验证码检验
    const auth = await ctx.auth(account, code);
    if (!auth) {
      return;
    }
    ...
    if (share) {
      try {
        const parent = await ctx.model.User.findOne({
          where: { share },
          include: [
            {
        ...
        const relation = ctx.model.Relation.build({
          user_id: user.id,
  ...
  public async auth() {
    ...
    // TODO 验证码的防刷
    // 发送验证码
    const code = ctx.helper.getRandom(4).toString();
    const result = await ctx.service.sms.send(account, code);
    if (result !== true) {
    ...
    ctx.success(ctx.app.config.env !== 'prod' ? { code } : null, '短信发送成功');
  }
  ...
  public async share() {
    ...
    // 我拉的人
    const count = await ctx.model.Relation.count({
      where: {
        $or: [
          { parent1_id: user.id },
          { parent2_id: user.id },
    ...
    // 我拉人获得的收入
    const award =
      (await ctx.model.Transaction.sum('cost_award', {
        where: {
