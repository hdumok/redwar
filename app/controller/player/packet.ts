...
export default class PacketController extends Controller {
  ...
  public async index() {
    ...
    if (packet.status === PacketStatus.Normal && new Date(packet.finished as string) < new Date()) {
  ...
  public async list() {
    ...
    const packets = await ctx.model.Packet.findAll({
      where: {
        room_id,
        created: {
          $lt: new Date(time),
      ...
      attributes: [ 'id', 'status', 'player_ids', 'lei', 'base_award', 'opened', 'created' ],
      limit,
      order: [[ 'created', 'desc' ]],
    });

    const result = packets.map(item => {
      const packet: any = item.get({ plain: true });
  ...
  public async send() {
    ...
    const room = await ctx.model.Room.findByPk(room_id);
    if (!room) {
      ctx.fail('房间不存在');
      return;
    }
    ...
    // 红包过期时间
    const packet_duration = await ctx.service.config.getPacketDuration();
    // 红包信息
    const packet = ctx.model.Packet.build({
    ...
    // 包超时结算
    setTimeout(this.packetExpired.bind(this, packet.id), new Date(packet.finished).getTime() - Date.now() + 3000);
  ...
  public async open() {
    ...
    try {
      ...
      if (packet.status === PacketStatus.Expired || new Date(packet.finished as string).getTime() < Date.now()) {
        ctx.fail('红包已过期');
        return;
      }
      ...
      // TODO 待修改到模型里
      // 是否抢过红包
      const open = await ctx.model.Transaction.findOne({
        where: {
          type: TransactionType.Qiang,
          packet_id: packet.id,
          user_id: user.id,
      ...
      // 判断我抢到的包的情况
      transaction.value = await ctx.service.rule.checkPacket(packet_item.award, packet.lei);
      ...
      switch (transaction.value) {
        ...
        case TransactionValue.ShunZi:
          ...
          if (packet.base_award >= 10 && packet.base_award <= 30) {
            // TODO 转化到配置service里
            const award_1030 = await ctx.service.config.getAward1030();
            transaction.spical_award = award_1030.spical;
          } else if (packet.base_award > 30) {
            const award_3060 = await ctx.service.config.getAward3060();
            transaction.spical_award = award_3060.spical;
          }
          transaction.remark = `用户 ${user.name} 特殊牌 ${transaction.value} 奖励 ${transaction.spical_award}`;
          break;
  ...
  private async packetMutliLei(packet_id: string) {
    // 多雷奖励
    const { ctx } = this;
    ...
    const transaction = ctx.model.Transaction.build({
      ...
      base_award: packet.base_award,
      cost_award,
      remark: `发包者 ${packet.user.name} 的红包 ${packet.lei_count}雷, 奖励 ${cost_award}`,
    });
  ...
  private async packetToRoom(packet_id: string) {
    ...
    const packet_item = await ctx.service.rule.getPacket(packet_id);
    ...
    try {
      ...
      transaction.value = await ctx.service.rule.checkPacket(transaction.packet_award, packet.lei);

      // 房主没有特殊牌奖励，也不应该有雷赔付
      transaction.remark = `房主 ${packet.room.name} 首包 ${transaction.packet_award}`;
  ...
  private async packetPresent(packet_id: string) {
    ...
    const room_packet = await ctx.model.Transaction.findOne({
      where: {
        type: TransactionType.Qiang,
        packet_id,
        user_id: '',
    ...
    // 给发包者的上级 分红
    let parents: any = [];
    const relation = await ctx.model.Relation.findOne({
    ...
    // 分红配置
    const award_present = await ctx.service.config.getAwardPresent();
    for (let i = 0, len = parents.length; i < len; i++) {
      const parent_id = parents[i];
      const parent_transaction = ctx.model.Transaction.build({
    ...
    try {
      await ctx.model.transaction(async t => {
        for (const parent_transaction of parents) {
          const user = await ctx.model.User.findByPk(parent_transaction.user_id, {
            transaction: t,
          });
          if (!user) continue;
  ...
  private async packetExpired(packet_id: string) {
    ...
    const transaction = ctx.model.Transaction.build({
      ...
      remark: `红包超时未领完, ID:${packet.id} 的红包内剩余${packet.all_turns - packet.turns}个红包共 ${packet.award} 退回发包者 ${
        packet.user.name
      }`,
    });
